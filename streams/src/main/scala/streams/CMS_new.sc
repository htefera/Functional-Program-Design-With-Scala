...
object ConfManagement:
  opaque type Viewers = Set[Person] //viewers is a type alias for a set of persons
...
def highlyRankedProlificAuthors(asking: Person): Set[Person] =
  def query(viewers: Viewers): Set[Person] =
    given Viewers=viewers //making a given instance passable
...
// querying the following question is not true actually because the logic is false
//conf.rankings.takeWhile(conf.score(_, Set()) > 80)
