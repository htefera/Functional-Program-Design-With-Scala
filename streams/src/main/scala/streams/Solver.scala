...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      initial #::: from(more.sortBy(_._2.length), explored ++ more.map(_._1).toSet)
    }
  }
// try to replace initial ++ from with initial #::: from
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart.filter{ case (block, listOfMoves) => block.b1 == goal && block.b2 == goal}
  }
